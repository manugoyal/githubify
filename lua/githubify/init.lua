...
function M.command(opts)
  local fargs = (opts and opts.fargs) or {}
...
function M.setup()
  if vim.fn.exists(":Githubify") == 2 then
    return
  end
  vim.api.nvim_create_user_command("Githubify", function(opts)
    M.command({ fargs = opts.fargs or {} })
  end, { nargs = "*" })
end
